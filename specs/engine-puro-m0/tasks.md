...
- [ ] T001 Criar `packages/engine/package.json` (`name: "@sdp/engine"`, `private: true`,
      `type: "module"`, scripts `test` e `test:coverage`, devDependencies `typescript`, `vitest`,
      `@vitest/coverage-v8`)
- [ ] T002 [P] Criar `packages/engine/tsconfig.json` (modo `strict`, `target: "ES2022"`,
      `module: "NodeNext"`, `noEmit: true` — sem build step em M0, conforme research.md §1)
- [ ] T003 [P] Criar `packages/engine/vitest.config.ts` com `coverage.provider: "v8"` e
      `thresholds: { lines: 80, functions: 80, branches: 80, statements: 80 }` (research.md §2)
- [ ] T004 Rodar `pnpm install` na raiz do repo e confirmar que `packages/engine` é reconhecido como
...
- [ ] T005 Definir os tipos centrais em `packages/engine/src/types.ts` (`NodeId`, `ComponentType`,
...
- [ ] T006 [P] Criar o catálogo de componentes em `packages/engine/src/catalog/components.ts` (11
      `ComponentType`, com throughput/latência p50-p99/custo, valores da tabela de data-model.md) —
      FR-014, D5
- [ ] T007 [P] Teste de sanidade do catálogo em `packages/engine/test/catalog/components.spec.ts`
      (11 tipos presentes; cada um com throughput > 0, `p50 < p99`, custo > 0)
- [ ] T008 Implementar validação estrutural em `packages/engine/src/graph/validate.ts` — nunca lança,
...
- [ ] T009 [P] Testes unitários de `validate.ts` em `packages/engine/test/graph/validate.spec.ts`
      (cada condição inválida produz o `Violation.type` correto; design válido não produz nenhuma)
- [ ] T010 Coverage pass: `validate.ts` — para cada ponto de decisão (checagem de aresta quebrada, ID
...
- [ ] T011 Criar `packages/engine/src/index.ts` exportando o esqueleto de `simulate()` — roda
...
- [ ] T012 [P] [US1] Testes de utilização (`ρ = λ/(c·μ)`) em
      `packages/engine/test/metrics/utilization.spec.ts`
- [ ] T013 [P] [US1] Testes de fila M/M/1 e Lei de Little, incluindo o caso `ρ ≥ 1` (saturado), em
      `packages/engine/test/metrics/queue.spec.ts`
- [ ] T014 [P] [US1] Testes da derivação de percentis p50/p95/p99 (research.md §3) em
      `packages/engine/test/metrics/latency.spec.ts`
- [ ] T015 [P] [US1] Testes de propagação de carga com normalização de peso de split (FR-018) em
      `packages/engine/test/graph/propagate.spec.ts`
- [ ] T016 [P] [US1] Testes de throughput/gargalo — nunca acima da carga ofertada nem da capacidade
      do caminho — em `packages/engine/test/metrics/throughput.spec.ts`
- [ ] T017 [US1] Design de referência A (nó único, ρ<1, conta à mão documentada no arquivo) em
      `packages/engine/test/reference-designs/design-a.spec.ts` (SC-001)
- [ ] T018 [US1] Design de referência B (gargalo no meio do caminho, conta à mão documentada) em
      `packages/engine/test/reference-designs/design-b.spec.ts` (SC-001)

### Implementation for User Story 1

- [ ] T019 [P] [US1] Implementar cálculo de utilização em `packages/engine/src/metrics/utilization.ts`
      (FR-003)
- [ ] T020 [P] [US1] Implementar fila M/M/1 + Lei de Little em `packages/engine/src/metrics/queue.ts`,
      com valor sentinela documentado para `ρ ≥ 1` (FR-004, Edge Cases)
- [ ] T021 [US1] Implementar derivação de percentis p50/p95/p99 em
      `packages/engine/src/metrics/latency.ts` (FR-005, research.md §3) — depende de T020
- [ ] T022 [US1] Implementar propagação de carga com normalização de peso em
      `packages/engine/src/graph/propagate.ts` (FR-002, FR-018) — depende de T005, T008
- [ ] T023 [US1] Implementar cálculo de throughput/gargalo em
      `packages/engine/src/metrics/throughput.ts` (FR-006, FR-007) — depende de T019, T022
- [ ] T024 [US1] Integrar propagação + utilização + fila + latência + throughput no pipeline de
      `packages/engine/src/index.ts` — depende de T019, T020, T021, T022, T023
- [ ] T025 [US1] Coverage pass: `queue.ts` — garantir que o ramo de saturação (`ρ ≥ 1`) e o ramo normal
      têm teste que mata mutação; escrever os testes faltantes
- [ ] T026 [US1] Coverage pass: `propagate.ts` — garantir que a normalização de peso (soma ≠ 100%) tem
      teste que mata mutação; escrever os testes faltantes
- [ ] T027 [US1] Coverage pass: `throughput.ts` — garantir que a seleção do nó gargalo tem teste que
...
- [ ] T028 [P] [US2] Teste de detecção de SPOF (réplicas < 2 no caminho crítico) em
      `packages/engine/test/graph/spof.spec.ts`
- [ ] T029 [P] [US2] Teste de detecção de nó órfão (vale zero; fora de cost/path/scores) em
      `packages/engine/test/graph/orphan-node.spec.ts`
- [ ] T030 [P] [US2] Teste de detecção de ciclo (nó membro do ciclo vs. nó a jusante) em
      `packages/engine/test/graph/cycle.spec.ts`
- [ ] T031 [P] [US2] Teste de exclusão de aresta assíncrona do cálculo de latência do usuário em
      `packages/engine/test/metrics/async-edge.spec.ts`

### Implementation for User Story 2

- [ ] T032 [US2] Implementar detecção de SPOF em `packages/engine/src/graph/static-analysis.ts`
      (FR-009 — `replicas ≥ 2` não-SPOF, decisão do autor)
- [ ] T033 [US2] Implementar detecção de nó órfão em `packages/engine/src/graph/static-analysis.ts`
      (FR-010) — mesmo arquivo de T032, sequencial
- [ ] T034 [US2] Implementar detecção de ciclo via DFS de 3 cores em
      `packages/engine/src/graph/static-analysis.ts` (FR-011) — mesmo arquivo, sequencial
- [ ] T035 [US2] Excluir o trecho de aresta assíncrona da soma de latência em
      `packages/engine/src/metrics/latency.ts` (FR-012) — depende de T021
- [ ] T036 [US2] Integrar as análises estáticas ao pipeline de `packages/engine/src/index.ts`
      (violations acumuladas; nó órfão excluído de cost/path/scores) — depende de T032, T033, T034,
      T035, T024
- [ ] T037 [US2] Coverage pass: `static-analysis.ts` — garantir que SPOF, órfão e ciclo têm cada um
      teste que mata mutação; escrever os testes faltantes
...
- [ ] T038 [P] [US3] Teste de cálculo de custo (N réplicas × custo unitário; soma no `monthlyTotal`)
      em `packages/engine/test/cost/calculate.spec.ts`

### Implementation for User Story 3

- [ ] T039 [US3] Implementar cálculo de custo por nó e total em `packages/engine/src/cost/calculate.ts`
      (FR-013) — depende de T006
- [ ] T040 [US3] Integrar custo ao pipeline de `packages/engine/src/index.ts` (excluindo nós órfãos,
      FR-010) — depende de T039, T036
- [ ] T041 [US3] Coverage pass: `calculate.ts` — garantir que o cálculo por nó e a soma total têm
      teste que mata mutação; escrever os testes faltantes
...
- [ ] T042 [P] [US4] Teste do efeito de cache (`carga_no_db`, `latência_efetiva`) em
      `packages/engine/test/metrics/cache.spec.ts`

### Implementation for User Story 4

- [ ] T043 [US4] Implementar efeito de cache sobre carga e latência em
      `packages/engine/src/metrics/cache.ts` (FR-008)
- [ ] T044 [US4] Integrar o efeito de cache ao pipeline de propagação/latência em
      `packages/engine/src/index.ts` — depende de T043, T024
- [ ] T045 [US4] Coverage pass: `cache.ts` — garantir que o ramo de hit e o de miss têm teste que mata
...
- [ ] T046 [P] Implementar o placeholder de `scores` (7 dimensões = `0`) em
      `packages/engine/src/index.ts` conforme FR-020 — depende de T024
- [ ] T047 [P] Teste do placeholder de `scores` (as 7 chaves de `Dimension` presentes, valor `0`) em
      `packages/engine/test/index.spec.ts`
- [ ] T048 Design de referência C — integra cache + custo + SPOF num único design, conta à mão
...
- [ ] T049 Teste de performance: grafo de 30 nós conclui em menos de 50 ms em
      `packages/engine/test/performance/scale.spec.ts` (SC-002)
- [ ] T050 [P] Verificar zero dependência de runtime — teste estático que falha se qualquer arquivo em
...
- [ ] T051 Rodar o passo a passo de [quickstart.md](quickstart.md) manualmente e confirmar que o
      exemplo funciona como documentado
- [ ] T052 Confirmar cobertura ≥80% via `pnpm --filter @sdp/engine test:coverage` (SC-003); se algum

...
- [X] T027 [P] `pnpm -r typecheck` (recursivo — cobre os 3 pacotes com código real: `engine`, `problems`, `web`; `packages/ui`/`packages/narrator` ainda são só README, nada a typecheck) limpo; `pnpm --filter engine test` — 159/159; `pnpm --filter problems test` — 7/7 (não tocado por esta feature, mas nunca tinha sido rodado nas fases anteriores — conferido agora); `pnpm --filter web test:coverage` propriamente **falha** (exit 1) por causa do flake pré-existente de `password.spec.ts` sinalizado em T001 (task em background aberta) — a mesma suíte com `--exclude "**/password.spec.ts"` dá 105/105 verdes, 100% em `connection-rules.ts`/`canvas-ui-catalog.ts`/`component-categories.ts`, 96.97% geral em `src/lib/**` (inclui `password.ts` a 0% no denominador — número conservador, não inflado); `pnpm --filter web build` limpo.
...
- [X] T029 (achado numa revisão adicional, não numerada originalmente) `apps/web/test/component-categories.spec.ts` só cobria os 11 `ComponentType` originais — os 33 valores novos de `CATEGORY_OF` tinham garantia de tipo (mapeiam pra ALGUMA `PaletteCategory` válida) mas nenhuma garantia de que fosse a categoria CERTA (ex. `dns` poderia compilar como `'Storage'` por engano e nada acusaria). `ALL_COMPONENT_TYPES` estendida pra 44 + 2 testes novos com a categoria exata de cada um dos 33 componentes novos (SC-001) — 6 testes no arquivo (era 4), 100% cobertura mantida.
